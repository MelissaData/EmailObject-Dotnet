<#
.SYNOPSIS
    Downloads the required components and then builds and runs MelissaEmailObjectWindowsDotnet

.DESCRIPTION
    This script uses the Melissa Updater to fetch the data file(s), DLL(s), and the C# wrapper,
    verifies the DLL(s) downloaded, then builds the .NET project and runs it against the supplied email address.
    ...
    Overall flow:
      ...
      4. Build the project, then run it (single test email address or interactive).
...
.EXAMPLE
    .\MelissaEmailObjectWindowsDotnet.ps1 -license "your-license"

.EXAMPLE
    .\MelissaEmailObjectWindowsDotnet.ps1 -email "info@melissa.com" -license "your-license"
#>
...
function CheckDLLs() {
  ...
  $FileMissing = $false
  if (!(Test-Path ("$BuildPath\mdEmail.dll"))) {
...
dotnet publish -f="net10.0" -c Release -o $BuildPath MelissaEmailObjectWindowsDotnet\MelissaEmailObjectWindowsDotnet.csproj
